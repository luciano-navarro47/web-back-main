...
```bash
web-back$ sam build --use-container
```
...
```bash
web-back$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
web-back$ sam local start-api
web-back$ curl http://localhost:3000/
```
...
```bash
web-back$ sam logs -n HelloWorldFunction --stack-name web-back --tail
```
...
```bash
web-back$ pip install -r tests/requirements.txt --user
# unit test
web-back$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
web-back$ AWS_SAM_STACK_NAME="web-back" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "web-back"
```
